

📌 Описание проекта

ISUP/ISAPI Bridge — это высоконадежный серверный модуль, который принимает данные с оборудования контроля доступа Hikvision (турникеты, терминалы распознавания лица, карточные считыватели), нормализует их и передает в 1С (Учет рабочего времени / ERP / УРВ / ЗУП).

Проект поддерживает два протокола:

🔵 ISUP v5 (бинарный протокол турникетов)
	•	Прямое TCP-подключение
	•	Парсинг бинарных пакетов
	•	Корректная генерация ACK
	•	Направление IN/OUT
	•	Карты, пин-коды, QR, комбинированные методы

🔵 ISAPI (HTTP Webhook, EventNotificationAlert)
	•	Терминалы FACE / CARD / MIXED
	•	Поддержка multipart (XML + JPEG snaphost)
	•	Извлечение cardNo, employeeNo, door, reader
	•	Автоматическое определение направления (IN/OUT)

⸻

⚙️ Архитектура проекта

/opt/isup_bridge/
│── main.py                   # Точка входа
│── core/
│     ├── processor.py        # Унификация событий и отправка в 1С
│     ├── storage.py          # Локальное хранилище pending-событий
│     ├── tenant_manager.py   # Управление объектами/1С
│     └── metrics.py          # Метрики
│── isup/
│     ├── isup_protocol.py    # Парсер ISUP v5 + генерация ACK
│     └── isup_server.py      # ISUP TCP сервер
│── isapi/
│     ├── isapi_client.py     # ISAPI Event parser + терминалы
│     └── isapi_server.py     # HTTP сервер ISAPI Webhook
│── utils/
│     └── logging_setup.py    # Единая настройка логирования
│── config/
│     └── config.yaml         # Конфигурация объектов, устройств, 1С
└── requirements.txt          # Зависимости проекта


⸻

🚀 Основные возможности

🔒 1. Поддержка всех типов Hikvision устройств
	•	Турникеты
	•	Контроллеры по ISUP
	•	Лицевые терминалы
	•	гибридные face+card устройства

🔁 2. Унификация событий

ISUP и ISAPI события приводятся к единому формату:

{
  "timestamp": "2025-01-21T10:23:41",
  "device_id": "343831383733390D",
  "event_type": "ACCESS",
  "direction": "IN",
  "card_number": "1234567890",
  "employee_number": "10078",
  "success": true
}

📨 3. Передача в 1С (HTTP Integration)
	•	HTTPS / HTTP
	•	Basic Auth
	•	retry логика (повторные попытки)
	•	configurable endpoint
	•	tenant-аутентификация
	•	поддержка нескольких объектов / филиалов

📦 4. Хранилище непереданных событий
	•	Каждый event сохраняется в storage/
	•	JSON формат
	•	Автоматическое удаление старых событий
	•	Постоянный авто-ретрай

📊 5. HTTP API для мониторинга

/health
/metrics
/tenants

🛡 6. Ограничение по IP (белый список)

ISUP/ISAPI принимаются только от разрешённых IP.

⸻

🧱 Требования
	•	Python 3.10+
	•	aiohttp
	•	pyyaml

Install:

pip install -r requirements.txt


⸻

🚀 Запуск приложения

python3 main.py

Bridge запускает:

Компонента	Порт	Описание
ISUP TCP сервер	:8001	Прием бинарных событий
ISAPI Webhook	:8002	XML + фото события
HTTP API	:8081	/health /metrics


⸻

📡 Настройка терминала Hikvision

В терминале:

Configuration → Event → Upload Events via HTTP → Enable

URL:

http://YOUR_SERVER_IP:8002/ISAPI/Event/notification/alert

Тип события:
	•	AccessControlEvent
	•	FaceMatchEvent
	•	CardEvent

⸻

🔧 Настройка ISUP контроллера

В Web UI:

Network → Advanced → ISUP

Указать:
	•	протокол v5
	•	адрес сервера → ваш сервер :8001
	•	heartbeat включен
	•	ACK режим — обязателен

⸻

🗂 Формат config.yaml

Объект включает:
	•	1С сервер
	•	ISUP контроллеры
	•	ISAPI терминалы
	•	настройки направления (entry/exit)

Пример:

objects:
  - object_id: "moscow_hq"
    name: "Главный офис Москва"
    c1:
      base_url: "https://1c.company.ru"
      endpoint: "/hs/access/events"
      username: "bridge"
      password: "pass"
    terminals:
      - terminal_id: "hq_main_entrance"
        ip_address: "192.168.10.10"
        direction: "in"


⸻

📈 Метрики (/metrics)

{
  "uptime_seconds": 3600,
  "events": {
    "received": 2048,
    "ok": 2033,
    "failed": 15,
    "pending": 6,
    "success_rate": 99.2
  }
}


⸻

🧪 Тестирование

Можно отправить тестовые события:

ISUP (через TCP):

nc YOUR_SERVER 8001 < sample_isup_packet.bin

ISAPI:

curl -X POST -d "@event.xml" http://YOUR_SERVER:8002/ISAPI/Event/notification/alert

⸻

🛠 Обновление сервера и systemd

1) Зафиксируйте новую структуру в каталоге `/opt/isup_bridge` (ветки `main`, `master` и `codex` должны содержать идентичное дерево выше).
2) Обновите зависимости:
   ```bash
   cd /opt/isup_bridge
   python3 -m venv .venv
   source .venv/bin/activate
   pip install -r requirements.txt
   ```
3) Убедитесь, что unit-файл systemd указывает на единый вход `main.py`:
   ```ini
   [Service]
   WorkingDirectory=/opt/isup_bridge
   ExecStart=/opt/isup_bridge/.venv/bin/python /opt/isup_bridge/main.py
   ```
4) Перезапустите службу:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl restart isup-bridge.service
   sudo systemctl status isup-bridge.service
   ```
5) Проверка:
   - `/health` на порту health_check из `config.yaml`.
   - Логи в `/opt/isup_bridge/logs/` (создаются автоматически).
   - `journalctl -u isup-bridge.service -f` должен показывать строки о запуске ISUP TCP и ISAPI Webhook серверов (например, сообщения
     `ISUP TCP server listening` и `ISAPI Webhook server started`).


⸻

🛠 Возможности расширения

Проект легко расширяется:
	•	отправка в Kafka
	•	хранение истории в PostgreSQL
	•	поддержка ZKTeco
	•	поддержка Dahua
	•	отправка фотографий в S3

⸻

👨‍💻 Автор eldar40 / Поддержка

Разработано по техническому заданию частной охранной организации “СОВА”
и адаптировано под федеральные объекты с несколькими филиалами.
